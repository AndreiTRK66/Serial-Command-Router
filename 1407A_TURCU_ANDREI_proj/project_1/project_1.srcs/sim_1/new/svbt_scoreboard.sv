class svbt_scoreboard extends svbt_base_unit;
    ...
    task check_packets();
        ...
        if ( (drv_packet.address == 3 && drv_packet.cmd inside {2'b00, 2'b01, 2'b10}) || 
           (drv_packet.address < 3 && slv_lock[drv_packet.address] == 1'b1 && drv_packet.cmd != 2'b11) ) begin
         
            $display("[%0t] [SCOREBOARD] Silent Drop (Slave blocat sau adresa eronata). Pachet ignorat.", $time);
            total_packets++;
            return; 
        end
